...
export const EventsendMessageDiscord: Event = {
  ...
  execute: (parameters: FieldGroup[]) => {
    ...
    const channelId = parameters
      .find((param) => param.id === 'channelDetails')
      ?.fields.find((field) => field.id === 'channelId')?.value;
    const message = parameters
      .find((param) => param.id === 'messageDetails')
      ?.fields.find((field) => field.id === 'message')?.value;

    if (channelId && message) {
      discordServiceMethodes
        .sendMessageToChannel(channelId, message, 'userId')
