...
export const EventSendMail: Event = {
  ...
  execute: (parameters: FieldGroup[]) => {
    ...
    const recipientEmailsField = emailGroup.fields.find(
      (field) => field.id === 'recipient_emails',
    );
    const subjectField = emailGroup.fields.find(
      (field) => field.id === 'email_subject',
    );
    const bodyField = emailGroup.fields.find(
      (field) => field.id === 'email_body',
    );
...
export const EventCheckFreezingTemperature: Event = {
  ...
  check: async (parameters: FieldGroup[]) => {
    ...
    const latitude = locationDetails.fields.find(
      (f) => f.id === 'latitude',
    )?.value;
    const longitude = locationDetails.fields.find(
      (f) => f.id === 'longitude',
    )?.value;
