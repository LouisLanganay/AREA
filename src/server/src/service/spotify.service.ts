...
class spotifyTools {
    ...
    async createPlaylist(userId, spotifyID, name, description, isPublic) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error('Failed to create playlist:', response.status, response.statusText, errorBody);
    ...
    async checkPlayers(userId): Promise<boolean> {
        const tokenData = await this.prisma.token.findUnique({
            where: {userId_provider: {userId, provider: 'spotify'}},
            select: {accessToken: true},
        });
        ...
        try {
            ...
            // const data = await response.json();
            console.log('A music is play');
            return true;
        } catch (error) {
            console.error('No music is play');
            return false;
        }
    }
        ...
        try {
            ...
            if (response.status != 200) {
                if (response.status === 403) {
                    throw new Error(`Spotify API error: You need to have a Premium account to do that`);
                }
                throw new Error(`Spotify API error: ${response.status} ${response.statusText}`);
            }
    ...
    async numberOfDevices(userId: string, numberMaxDevice: number): Promise<boolean> {
        const tokenData = await this.prisma.token.findUnique({
            where: { userId_provider: { userId, provider: 'spotify' } },
            select: { accessToken: true },
        });
        ...
        try {
            ...
            console.log(`Number of connected devices: ${numberOfDevices}. Check for ${numberMaxDevice}`);
...
const spotiTools = new spotifyTools(
    prismaService,
);
...
export const EventCreatePlaylist: Event = {
    ...
    execute: (parameters: FieldGroup[]) => {
        ...
        if (name && description && spotifyID) {
            ...
            console.error('Missing required parameters: name or description or spotifyID');
            return false;
        }
    },
};
